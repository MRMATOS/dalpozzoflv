...
import { Loader2, Plus, Save, ChevronRight, ChevronDown, Trash2 } from 'lucide-react';
...
const ProdutosTab = () => {
  ...
  const handleCreateVariation = (produtoPaiId: string) => {
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            Produtos Principais ({principais}) - Total com variações ({totalVariacoes})
          </CardTitle>
              ...
              {produtos?.map((produto) => (
                <React.Fragment key={produto.id}>
                    ...
                    <TableRow className="bg-blue-50">
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          <Button
                            size="sm"
                            onClick={() => handleCreateVariation(produto.id)}
                            disabled={!newVariation.nome_variacao || createVariationMutation.isPending}
                          >
                            {createVariationMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                            Salvar
                          </Button>
                          <Button
                            variant="outline"
                  ...
                  {expandedProducts.has(produto.id) && produto.variacoes?.map((variacao) => (
                    <TableRow key={variacao.id} className="hover:bg-muted/50">
                      <TableCell></TableCell>
                      <TableCell className="pl-8">
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleDeleteClick(variacao)}
                            className="text-red-600 hover:text-red-700"
                          >
                            <Trash2 className="w-4 h-4" />
                          </Button>
